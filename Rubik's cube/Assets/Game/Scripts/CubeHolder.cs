 ...
 */
    ...
    public bool CheckSolved()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            center = Pieces[Side.ALLSIDE[i][0]].name;
            for (int j = 1; j < 9; j++)
            {
                currentPiece = Pieces[Side.ALLSIDE[i][j]];
                if (!currentPiece.name.Contains(center) ||
    ...
    {
        if(side.Length > 10 && side[0] != piece)
        {
            Pieces[side[0]].transform.parent = Pieces[piece].transform;
        }
